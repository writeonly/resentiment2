...
class MetaBF {
  ...
  protected def rm(w: Int, pre: String, suf: String, seq: FString*): FString =
    FString((sep) => jm(h(w, pre)(sep), vm.mkf(seq)(sep), h(w, suf)(sep)))

  protected def h(i: Int, s: String, n: Int): FString =
    FString((_) => {
      val shift = i - head
      head = i
      sh(shift) + s * n
  ...
  protected def hm(w: Int, in: String, seq: FString*): FString =
    FString((sep) => jmhs(h(w)(sep), vm.mkfe(seq, h(w, in))(sep)))

  protected def hm(w: Int, seq: FString*): FString = hm(w, "", seq: _*)

  protected def hs(w: Int, in: String, seq: FString*): FString =
    FString((sep) => jmhs(h(w)(sep), vs.mkfe(seq, h(w, in))(sep)))
  ...
  private def signn(i: Int, p: String, n: String): String =
    sign(i, p, n) * i.abs
